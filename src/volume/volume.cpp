...
image ray_cast(volume const& vol)
{
    ...
    for(int i = 0 ; i < Ny; i++)
      ...
      for(int j = 0 ; j < Nz; j++)
        ...
        for(int k = 0 ; k < Nx; k++)
          {
          val1  = val1 + 2.0 *  vol(k,i,j) * dx ;  //question 8
          val2  = val2 - 5.0 * vol(k,i,j) * dx; //question 9
          }
        im(i,j) = val1 + val2;  //emission + attenuation
        }
      }
    return im;
}
...
void volume::load(std::string const& filename)
{
    ...
    std::ifstream stream( data_dir + filename, std::ifstream::in );
